...
const DonutChart3: React.FC<DonutChartProps> = ({ modules, totalUsers }) => {
  ...
  const chartOptions: ChartOptions<'doughnut'> = {
    ...
    interaction: {
      mode: 'nearest' as const, // Detects the nearest chart element
      intersect: true,
    },
  ...
  const centerLabel = hoveredIndex !== null
    ? {
        ...
        middle: `${modules[hoveredIndex].value} users`,
        bottom: `${((modules[hoveredIndex].value / totalUsers) * 100).toFixed(2)}%`,
      }
    : {
        top: 'Total',
        middle: `${totalUsers} users`,
        bottom: '',
      };
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-xs font-semibold text-navy text-center px-2 pointer-events-none">
        ...
        <div className="leading-tight">{centerLabel.middle}</div>
        {centerLabel.bottom && <div className="leading-tight">{centerLabel.bottom}</div>}
      </div>
    </div>
  );
};
